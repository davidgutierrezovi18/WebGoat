...
public class JWTRefreshEndpoint extends AssignmentEndpoint {
  ...
  public ResponseEntity<AttackResult> checkout(
    ...
    try {
      // CAMBIO MÍNIMO 1: Sustitución de .parse() por .parseClaimsJws() para validar firma
      Claims claims = Jwts.parser().setSigningKey(JWT_PASSWORD).parseClaimsJws(token.replace("Bearer ", "")).getBody();
      String user = (String) claims.get("user");
      if ("Tom".equals(user)) {
        // Mantenemos la lógica de verificación de cabecera original si es necesaria
  ...
  public ResponseEntity newToken(
    ...
    String refreshToken;
    try {
      // CAMBIO MÍNIMO 2: Sustitución de .parse() por .parseClaimsJws() y acceso directo al body
